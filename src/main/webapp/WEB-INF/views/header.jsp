					...
					<div class="move">
						...
						<c:if test="${sessionScope.memberSession.identity == '점주'}">
							<c:if test="${sessionScope.memberSession.permitConfirm == -1}">
								<button class="btn btn-secondary btn-register btn-sm" onclick='location.href = "registerForm.do"'>가맹점 문의</button>
							</c:if>
							<c:if test="${sessionScope.memberSession.permitConfirm == 0}">
								<button class="btn btn-secondary btn-register btn-sm">승인대기 중..</button>
							</c:if>
							<c:if test="${sessionScope.memberSession.permitConfirm == 1}">
								<button class="btn btn-secondary btn-register btn-sm" onclick="location.href='mypcUpdateForm.do'">가맹점 수정</button>
							</c:if>
						</c:if>
					</div>
