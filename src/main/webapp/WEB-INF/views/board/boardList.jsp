...
<div class="form_line">
	...
	<!-- 시작 페이지가 pagePerBlock보다 크면 앞에 보여줄 것이 있다 -->
		<c:if test="${pb.startPage > pb.pagePerBlock }">
			<li><a class="margin-btn btn btn-outline-primary" href="boardList.do?pcno=${board.pcno}&pageNum=1&searchKey=${board.searchKey}&searchValue=${board.searchValue}">
				<i class="fas fa-angle-double-left"></i></a></li>
			<li><a class="margin-btn btn btn-outline-primary" href="boardList.do?pcno=${board.pcno}&pageNum=${pb.startPage-1}&searchKey=${board.searchKey}&searchValue=${board.searchValue}">
				<i class="fas fa-angle-left"></i></a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
			<c:if test="${pb.currentPage==i }">
				<li><a class="margin-btn btn btn-outline-primary active" href="boardList.do?pcno=${board.pcno}&pageNum=${i}&searchKey=${board.searchKey}&searchValue=${board.searchValue}">${i}</a></li>
			</c:if>
			<c:if test="${pb.currentPage!=i }">
				<li><a class="margin-btn btn btn-outline-primary" href="boardList.do?pcno=${board.pcno}&pageNum=${i}&searchKey=${board.searchKey}&searchValue=${board.searchValue}">${i}</a></li>
			</c:if>
		</c:forEach>
		<!-- 보여줄 것이 남아있는 경우에는 endPage보다 totalPage가 큰경우 -->
		<c:if test="${pb.endPage < pb.totalPage }">
			<li><a class="margin-btn btn btn-outline-primary" href="boardList.do?pcno=${board.pcno}&pageNum=${pb.endPage+1}&searchKey=${board.searchKey}&searchValue=${board.searchValue}">
				<i class="fas fa-angle-right"></i></a></li>
			<li><a class="margin-btn btn btn-outline-primary" href="boardList.do?pcno=${board.pcno}&pageNum=${pb.totalPage}&searchKey=${board.searchKey}&searchValue=${board.searchValue}">
